...
def get_hyperlink(hyperlinks, mapping_name):
    ...
    for i, item in enumerate(hyperlinks):
        item = item.split('/')[-1] if item not in "./" else item
        str_hyperlink += "- "
...
def plot_keywords(one_key):
    list_type = [[x] for x in fined_taxonomy[one_key]]
    index = taxonomy_key2id[one_key]
    disc = f"This page categorizes the literature by the **{mapping_name[one_key]}**."
    plot_content(index=index, keys=["keywords"], dir_path=dir_path, disc=disc,
                 list_type=list_type, mapping_name=mapping_name)

# check repetition
check_repetition()

init_dir_path = ["./", "taxonomy/year",  "taxonomy/author", "taxonomy/venue"]
taxonomy_key = [x for x in list(fined_taxonomy.keys()) if x not in ['conference', 'journal', 'preprint']]
taxonomy_key2id = {x:i + len(init_dir_path) for i, x in enumerate(taxonomy_key)}
...
dir_path = init_dir_path + ["taxonomy/" + x for x in taxonomy_key
                            if x not in ['conference', 'journal', 'preprint']]

# 0 Home / Paper Type
list_type = [[x] for x in fined_taxonomy["paper_type"]]
indexs = 0
disc = f"This page categorizes the literature by the **{mapping_name['paper_type']}**."
plot_content(index=0, keys=["keywords"], dir_path=dir_path, disc=disc, list_type=list_type,
...
plot_content(index=index, keys=["booktitle", "journal"], dir_path=dir_path, disc=disc,
             list_type=list_type, mapping_name=mapping_name)

for one_key in taxonomy_key:
    plot_keywords(one_key)

# # 3 Task
# list_type = [[x] for x in fined_taxonomy["task"]]
# index = taxonomy_key2id["task"]
...
# index = taxonomy_key2id["paradigm"]
...
# index = taxonomy_key2id["framework"]
